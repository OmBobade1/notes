...
`00`-`01` — make sure the fundamentals and device landscape are solid before touching the target.
`02` — plan the engagement around the phases; know what "done" looks like.
`03` — passive/OSINT recon first, zero footprint on target systems.
`05`-`07` — scan, then enumerate every open service found, one at a time.
`09` — attempt credential attacks on whatever services allow login.
`11`, `13`-`14` — exploit, gain a foothold, escalate, move laterally, persist.
`10`, `08` — as needed, intercept traffic or craft custom packets for specific tests.
`06`, `12` — apply evasion techniques throughout, wherever detection/blocking becomes a factor.
`15` — a separate, architecture-level review pass, independent of active exploitation.
`16`-`18` — Wi-Fi as its own, self-contained track, run whenever wireless is in scope.
---
📖 Full Module Index
#	File	Covers
