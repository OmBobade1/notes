...
Theory-to-practice track: 19 numbered modules (`00`-`18`) covering networking fundamentals through Wi-Fi exploitation, plus hands-on wargame write-ups applying it.

## 📂 Structure
```
01-network security/
├── network-module-00-osi-tcpip.md          ... through ...
├── network-module-18-wpa3-dragonblood.md   (19 theory modules)
└── bandit/                                  ← hands-on OverTheWire wargame write-ups
```
...
| **Want to see this applied hands-on, level by level** | `bandit/` |
...
| Folder | Covers |
|---|---|
| `bandit/` | OverTheWire Bandit — level-by-level write-ups applying the recon/enumeration/privesc concepts from the modules above to a real wargame |

## How to use this
Each module is self-contained but built to be read in order — later modules assume earlier ones as known context and explicitly reference them by number. Every module includes real commands, the attacker's specific reasoning (not just "what," but "why" and "when"), and mitigation. The `bandit/` folder is where that theory gets applied hands-on, level by level.
